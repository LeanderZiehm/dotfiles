...
# Base QEMU launcher - never change this
qemu_launch() {
    local NAME="$1"
    local ISO="$2"
    local IMG="$3"
    local MEM="${4:-2G}"    # default RAM
    local CPU="${5:-2}"     # default CPU cores
    local NET="${6:-user,hostfwd=tcp::2222-:22}" # default networking
...
# Example VM config function
qarch() {
    local NAME="arch_vm"
    local ISO="$HOME/Documents/archlinux-2025.05.01-x86_64.iso"
    local IMG="$HOME/vms/arch_vm.qcow2"
    local MEM="4G"
    local CPU="4"
